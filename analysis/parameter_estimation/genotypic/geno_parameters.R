...
# param1 <- foreach(i=1:length(traits), .combine=rbind ) %dopar% {
    ...
    # fit <-asreml(fixed= eval(parse(text=t)) ~ Pop,
                 ...
                 # maxiter=500,
                 # data=pheno)
    # saveRDS(fit, file=paste(t,".rds", sep=""))
    # vars <- summary(fit)$varcomp  # get variance components
    # h2 <- signif(vars[2,2]/sum(vars[,2]), 2)  # calculate heritability
    # r <- signif(sum(vars[1:2,2])/sum(vars[,2]), 2)  # calculate repeatabilit
    # return(data.frame(trait=traits[i],h2=h2,r=r))
 # }
# Fit univariate model for estimatign yield heritability
...
#write.csv(param, "parameters.csv", quote=F, row.names=F )
...
# write.csv(cor.g, "genetic_correlations.csv",  row.names=F, quote=F)

# Calculate index heritabilty and export G
...
colnames(b) <- "weight"
h_i <- (t(b) %*% G %*% b) / (t(b) %*% P %*% b)
write.csv(b, "index_weights.csv", quote=F)